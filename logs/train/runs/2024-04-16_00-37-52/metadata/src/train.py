...
def train(cfg: DictConfig) -> Tuple[dict, dict]:
    ...
    # Define the database URI and the artifact location
    #db_uri = "sqlite:///mlflow.db"
    #artifact_location = os.path.abspath("./mlruns")

    # Set the tracking URI to use the SQLite database
    #mlflow.set_tracking_uri(db_uri)

    # Initialize MLflow tracking
    #mlflow.set_experiment("My PyTorch Experiment")
    #mlflow.start_run()

    mlflow.set_tracking_uri("file://" + os.path.abspath("./mlruns"))
    mlflow.set_experiment("mnist_pytorch_lightning")

    mlflow.start_run()

    # Log GPU metadata, parameters, and start training
    #utils.log_gpu_memory_metadata()
    #gpu_log_file = "./gpu_memory_metadata.txt"
    #if not os.path.exists(gpu_log_file):
    #    open(gpu_log_file, 'w')
    #mlflow.log_artifact(gpu_log_file)

    # Assuming cfg is a dict-like config, log parameters
    ...
    # After training, log metrics and model
